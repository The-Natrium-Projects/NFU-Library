 ...
 */
//@Deprecated
public class DebugTargetSetterItem extends NFUItem
{
	...
	private Mob getAttacker(ItemStack stack, Level level)
	{
		...
		{
			Entity attackerEntity = NFUEntityStatics.getEntityByUUID(level, stack.getTag().getUUID("attacker"));
	...
	public InteractionResult interactLivingEntity(Player player, LivingEntity target, InteractionHand hand) 
	{
		...
		if (target instanceof Mob mob && mob.getTarget() == player)
		{
			...
			NFUMiscStatics.printToScreen("Removed the target of [" + mob.getName().getString() + "].", player);
			if (attacker != null)
			{
