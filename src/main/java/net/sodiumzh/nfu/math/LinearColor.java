...
public class LinearColor
{
	...
	public static EntityDataSerializer<LinearColor> getEntityDataSerializer() {
		return NFUEntityDataSerializers.LINEAR_COLOR.get();
	};
	...
	public Vec3 toHLS()
	{
		...
		double c = NFUMathStatics.max(r, g, b) - NFUMathStatics.min(r, g, b);
		double lightness = (NFUMathStatics.max(r, g, b) + NFUMathStatics.min(r, g, b)) / 2d;
		double saturation = 0;
		if (toCode() != 0xffffff)
